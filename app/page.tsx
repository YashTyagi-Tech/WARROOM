...
// ===== Components =====
function AITextBlock({ text, color="#8aaccc", fontSize=11 }: { text:string; color?:string; fontSize?:number }) {
          ...
          <div key={i} style={{ display:"flex",flexDirection:"column",gap:5 }}>
            {lines.map((line,j)=>line.startsWith("• ")?(
              <div key={j} style={{ display:"flex",gap:8,alignItems:"flex-start" }}>
                ...
                <span style={{ fontSize,color,lineHeight:1.85,fontFamily:"'Share Tech Mono',monospace" }}>{line.replace(/^•\s*/,"")}</span>
              </div>
            ):(
              <p key={j} style={{ fontSize,color,lineHeight:1.85,fontFamily:"'Share Tech Mono',monospace",margin:0 }}>{line}</p>
            ))}
          </div>
        );
        return (
...
function NationThreatMatrix({ nationThreats=[], events=[] }: { nationThreats?:any[]; events?:Event[] }) {
  ...
  return (
    <div style={{ background:"#070707",border:"1px solid #141414",borderRadius:4,overflow:"hidden",fontFamily:"'Share Tech Mono','Courier New',monospace" }}>
      {/* Header */}
      <div style={{ padding:"10px 14px",borderBottom:"1px solid #111",background:"#050505",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
        <div>
          <div style={{ fontSize:7,letterSpacing:5,color:"#252525",marginBottom:3 }}>GLOBAL THREAT INDEX</div>
          <div style={{ fontSize:isMobile?9:11,fontWeight:700,color:"#3a3a3a",letterSpacing:2 }}>NATION THREAT MATRIX</div>
        </div>
      ...
      {/* Summary strip */}
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",borderBottom:"1px solid #0d0d0d" }}>
        {[{label:"AVG THREAT",value:avgThreat,color:threatColor(avgThreat),suffix:"%"},{label:"CRITICAL",value:critCount,color:"#ef4444",suffix:" states"},{label:"HIGH ALERT",value:highCount,color:"#f97316",suffix:" states"}].map((s,i)=>(
          <div key={i} style={{ padding:isMobile?"7px 8px":"10px 14px",borderRight:i<2?"1px solid #0d0d0d":"none",textAlign:"center" }}>
            <div style={{ fontSize:7,color:"#252525",letterSpacing:3,marginBottom:4 }}>{s.label}</div>
            <div style={{ fontSize:isMobile?16:20,fontWeight:900,color:s.color,lineHeight:1 }}>{s.value}<span style={{ fontSize:8,fontWeight:400,color:"#2a2a2a" }}>{s.suffix}</span></div>
          </div>
        ))}
      </div>
      {/* Rows */}
      <div>
        {nations.length===0?(
          <div style={{ padding:32,textAlign:"center",color:"#1c1c1c",fontSize:9,letterSpacing:3 }}>LOADING...</div>
        ):nations.map((n,i)=>{
          const color = threatColor(n.threat);
          const label = threatLabel(n.threat);
          const isTop = i<3;
          return (
            <div key={n.code} style={{ display:"flex",alignItems:"center",gap:isMobile?8:12,padding:isMobile?"8px 10px":"10px 14px",background:isTop?`${color}08`:"transparent",borderBottom:"1px solid #0d0d0d",borderLeft:`2px solid ${isTop?color:"transparent"}` }}>
              <div style={{ width:18,height:18,borderRadius:3,background:"#0a0a0a",border:"1px solid #1c1c1c",display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,color:i<=2?color:"#2a2a2a",fontFamily:"'Share Tech Mono',monospace",fontWeight:900,flexShrink:0 }}>{i+1}</div>
              <div style={{ width:isMobile?24:30,height:isMobile?16:20,borderRadius:2,background:"#0d0d0d",border:`1px solid ${isTop?color+"44":"#1c1c1c"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:isMobile?6.5:7.5,color:isTop?color:"#3a3a3a",fontFamily:"'Share Tech Mono',monospace",fontWeight:700,flexShrink:0,letterSpacing:0.5 }}>{n.code}</div>
              <div style={{ flex:1,minWidth:0 }}>
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4 }}>
                  <span style={{ fontSize:isMobile?8.5:10,color:isTop?"#c4d4e8":"#4a4a4a",fontFamily:"'Share Tech Mono',monospace",fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginRight:6 }}>{n.name}</span>
                ...
                <div style={{ height:4,background:"#0d0d0d",borderRadius:2,overflow:"hidden",position:"relative" }}>
                  {[25,50,75].map(tick=>(<div key={tick} style={{ position:"absolute",top:0,left:`${tick}%`,width:1,height:"100%",background:"#1a1a1a",zIndex:1 }}/>))}
              ...
              <div style={{ textAlign:"right",flexShrink:0,minWidth:isMobile?28:36 }}>
                ...
                <div style={{ fontSize:7,color:"#2a2a2a",marginTop:1 }}>/100</div>
              </div>
            </div>
          );
        })}
      ...
      {/* Footer */}
      <div style={{ padding:"7px 14px",borderTop:"1px solid #0a0a0a",background:"#030303",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
        <div style={{ fontSize:7,color:"#1a1a1a" }}>Claude AI + Live Event Analysis</div>
        <div style={{ fontSize:7,color:"#1a1a1a",letterSpacing:2 }}>{nations.length} MONITORED</div>
      </div>
    </div>
  );
}
...
function ChartTip({active,payload,label}:{active?:boolean;payload?:any[];label?:string}){
  ...
  return <div style={{background:"#0a0a0a",border:"1px solid #222",borderRadius:3,padding:"7px 11px",fontSize:10,fontFamily:"'Share Tech Mono',monospace"}}><div style={{color:"#444",marginBottom:3}}>{label}</div>{payload.map((p,i)=><div key={i} style={{color:p.color||"#e2e8f0"}}>${fmt(p.value)}</div>)}</div>;
}
...
function CryptoCard({coin,chart,onClick,selected}:{coin:CryptoCoin;chart:any[];onClick:()=>void;selected:boolean}){
  ...
  return <div onClick={onClick} style={{padding:"13px 15px",borderRadius:4,background:selected?"#111":"#0a0a0a",border:`1px solid ${selected?c:"#1c1c1c"}`,cursor:"pointer",transition:"border-color .15s",position:"relative",overflow:"hidden"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=c;}} onMouseLeave={e=>{e.currentTarget.style.borderColor=selected?c:"#1c1c1c";}}>
    {selected&&<div style={{position:"absolute",top:0,left:0,right:0,height:2,background:c}}/>}
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <div style={{width:28,height:28,borderRadius:3,background:"#111",border:`1px solid ${c}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,color:c,fontWeight:900,fontFamily:"'Share Tech Mono',monospace"}}>{coin.symbol}</div>
        <div><div style={{fontSize:11,fontWeight:700,color:"#e2e8f0",fontFamily:"'Share Tech Mono',monospace"}}>{coin.id}</div><div style={{fontSize:8,color:"#333"}}>{coin.name}</div></div>
      </div>
      <div style={{textAlign:"right"}}><div style={{fontSize:13,fontWeight:800,color:"#e2e8f0",fontFamily:"'Share Tech Mono',monospace"}}>${fmt(coin.price,coin.price>100?0:2)}</div><div style={{fontSize:9,color:up?"#22c55e":"#ef4444"}}>{up?"+":"-"}{Math.abs(coin.change).toFixed(2)}%</div></div>
    </div>
    <Sparkline data={chart} color={c} height={42}/>
    {coin.marketCap>0&&<div style={{display:"flex",gap:12,marginTop:6}}><div style={{fontSize:7.5,color:"#333"}}><span style={{color:"#444"}}>MCap</span> {fmtB(coin.marketCap)}</div><div style={{fontSize:7.5,color:"#333"}}><span style={{color:"#444"}}>Vol</span> {fmtB(coin.volume)}</div></div>}
  </div>;
}

function StockRow({s}:{s:Stock}){
  const up=s.change>=0;
  return <div style={{display:"flex",alignItems:"center",padding:"9px 13px",borderBottom:"1px solid #111",transition:"background .12s"}} onMouseEnter={e=>e.currentTarget.style.background="#0f0f0f"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
    <div style={{width:6,height:6,borderRadius:1,background:s.color,marginRight:10,flexShrink:0}}/>
    <div style={{flex:1}}><div style={{fontSize:10,color:"#c4d4e8",fontFamily:"'Share Tech Mono',monospace"}}>{s.id}</div><div style={{fontSize:8,color:"#333"}}>{s.name}</div></div>
    <div style={{textAlign:"right"}}><div style={{fontSize:11,fontWeight:700,color:"#e2e8f0",fontFamily:"'Share Tech Mono',monospace"}}>{s.price>0?`$${fmt(s.price,s.price>100?0:2)}`:"—"}</div><div style={{fontSize:9,color:up?"#22c55e":"#ef4444"}}>{up?"+":"-"}{Math.abs(s.change).toFixed(2)}%</div></div>
  </div>;
}

function VideoCard({v,onPlay,playing}:{v:VideoItem;onPlay:(v:VideoItem)=>void;playing:boolean}){
  return <div onClick={()=>onPlay(v)} style={{borderRadius:4,overflow:"hidden",background:"#0a0a0a",border:`1px solid ${playing?"#ef4444":"#1c1c1c"}`,cursor:"pointer",transition:"border-color .15s",position:"relative",flexShrink:0}} onMouseEnter={e=>{if(!playing)e.currentTarget.style.borderColor="#333";}} onMouseLeave={e=>{if(!playing)e.currentTarget.style.borderColor="#1c1c1c";}}>
    ...
    <div style={{padding:"8px 10px"}}><div style={{fontSize:9,color:"#c4d4e8",fontFamily:"'Share Tech Mono',monospace",lineHeight:1.4,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{v.title}</div><div style={{fontSize:7.5,color:"#333",marginTop:4}}>{v.channel}</div></div>
  </div>;
}

function NewsCard({item,big,onClick}:{item:NewsItem;big?:boolean;onClick?:(item:NewsItem)=>void}){
  return <div onClick={onClick?()=>onClick(item):undefined} style={{display:"block",textDecoration:"none",padding:"10px 12px",borderRadius:4,background:"#0a0a0a",border:"1px solid #1c1c1c",marginBottom:6,transition:"border-color .15s",cursor:onClick?"pointer":"default"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=onClick?"#818cf8":"#333";}} onMouseLeave={e=>{e.currentTarget.style.borderColor="#1c1c1c";}}>
...
function EventCard({ev,isNew}:{ev:Event;isNew?:boolean}){
  ...
  return <div style={{padding:"8px 10px",marginBottom:5,borderRadius:3,background:"#0a0a0a",border:`1px solid ${isNew?"#1c1c1c":"#111"}`,borderLeft:`3px solid ${c}`}}>
...
function DefconDisplay({level=3}:{level?:number}){
  ...
  return <div style={{padding:"12px 14px"}}>
    ...
    <div style={{display:"flex",gap:isMobile?4:5,justifyContent:"center",marginBottom:10}}>
      {lvls.map(l=><div key={l.n} style={{width:isMobile?28:33,height:isMobile?28:33,borderRadius:3,background:"#0a0a0a",border:`1px solid ${l.n===level?l.c:"#222"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:isMobile?11:13,fontWeight:800,color:l.n===level?l.c:"#333",fontFamily:"'Share Tech Mono',monospace"}}>{l.n}</div>)}
    </div>
...
function WorldMap({hotspots,events,selectedSpot,onSelect}:{hotspots:Hotspot[];events:Event[];selectedSpot:Hotspot|null;onSelect:(s:Hotspot|null)=>void}){
  ...
  return <div style={{position:"relative",width:"100%",height:"100%",background:"#050505"}}>
    <svg viewBox="0 0 1000 500" style={{width:"100%",height:"100%",display:"block"}} preserveAspectRatio="xMidYMid meet">
      <defs><filter id="glow"><feGaussianBlur stdDeviation="2" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter><pattern id="mgrid" width="50" height="50" patternUnits="userSpaceOnUse"><path d="M50 0L0 0 0 50" fill="none" stroke="rgba(255,255,255,0.02)" strokeWidth="0.5"/></pattern></defs>
      <rect width="1000" height="500" fill="#050505"/><rect width="1000" height="500" fill="url(#mgrid)"/>
      {[-60,-30,0,30,60].map(lat=>{const y=(90-lat)*(500/180);return<line key={lat} x1={0} y1={y} x2={1000} y2={y} stroke="rgba(255,255,255,0.025)" strokeWidth="0.5" strokeDasharray="3,6"/>;})}{[-120,-60,0,60,120].map(lng=>{const x=(lng+180)*(1000/360);return<line key={lng} x1={x} y1={0} x2={x} y2={500} stroke="rgba(255,255,255,0.025)" strokeWidth="0.5" strokeDasharray="3,6"/>;})}<g fill="#141414" stroke="#252525" strokeWidth="0.8">{LAND.map((d,i)=><path key={i} d={d}/>)}</g>
      ...
      {tip&&<g><rect x={Math.min(tip.x+10,810)} y={Math.max(tip.y-28,5)} width={175} height={50} rx={3} fill="rgba(5,5,5,0.98)" stroke="rgba(255,255,255,0.06)" strokeWidth="0.7"/>{tip.ev&&<><text x={Math.min(tip.x+18,818)} y={Math.max(tip.y-10,22)} fontSize="7.5" fill="#d4e4f4" fontFamily="'Share Tech Mono',monospace" style={{pointerEvents:"none"}}>{tip.ev.headline?.slice(0,28)}</text><text x={Math.min(tip.x+18,818)} y={Math.max(tip.y+4,34)} fontSize="7" fill="#444" fontFamily="'Share Tech Mono',monospace" style={{pointerEvents:"none"}}>{tip.ev.country} · {tip.ev.timeAgo}</text></>}{tip.spot&&<><text x={Math.min(tip.x+18,818)} y={Math.max(tip.y-10,22)} fontSize="8" fill={TCOL[tip.spot.type]} fontFamily="'Share Tech Mono',monospace" style={{pointerEvents:"none"}}>{tip.spot.name}</text><text x={Math.min(tip.x+18,818)} y={Math.max(tip.y+4,34)} fontSize="7" fill="#444" fontFamily="'Share Tech Mono',monospace" style={{pointerEvents:"none"}}>Intensity: {Math.round((tip.spot.intensity||tip.spot.base)*100)}%</text></>}</g>}
    </svg>
  </div>;
}
...
function VideoModal({video,onClose}:{video:VideoItem|null;onClose:()=>void}){
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.93)",zIndex:9990,display:"flex",alignItems:"center",justifyContent:"center",padding:"16px"}} onClick={onClose}>
    <div style={{width:"min(860px,96vw)",background:"#0a0a0a",border:"1px solid #222",borderRadius:4,overflow:"hidden"}} onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",borderBottom:"1px solid #1c1c1c"}}>
        <div><div style={{fontSize:7,color:"#ef4444",letterSpacing:3,marginBottom:2,fontFamily:"'Share Tech Mono',monospace"}}>NOW PLAYING</div><div style={{fontSize:11,color:"#d4e4f4",fontFamily:"'Share Tech Mono',monospace"}}>{video.title?.slice(0,70)}</div></div>
      ...
      <div style={{padding:"10px 14px",display:"flex",gap:12,alignItems:"center"}}><div style={{flex:1}}><div style={{fontSize:9,color:"#444"}}>{video.channel}</div></div><a href={video.url} target="_blank" rel="noreferrer" style={{fontSize:8,color:"#38bdf8",textDecoration:"none",border:"1px solid #38bdf822",padding:"3px 10px",borderRadius:3,fontFamily:"'Share Tech Mono',monospace"}}>OPEN ON YOUTUBE</a></div>
    </div>
  </div>;
}
...
function NewsSummaryModal({item,onClose}:{item:NewsItem;onClose:()=>void}){
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.95)",zIndex:9990,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={onClose}>
    <div style={{width:"min(680px,95vw)",background:"#0a0a0a",border:"1px solid #818cf8",borderRadius:4,overflow:"hidden",animation:"fadeUp .2s ease",maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
      <div style={{padding:"12px 16px",borderBottom:"1px solid #1c1c1c",display:"flex",gap:10,alignItems:"flex-start",position:"sticky",top:0,background:"#0a0a0a",zIndex:1}}>
        <div style={{flex:1}}><div style={{fontSize:7,color:"#818cf8",letterSpacing:3,marginBottom:5,fontFamily:"'Share Tech Mono',monospace"}}>AI INTELLIGENCE BRIEF</div><div style={{fontSize:11,color:"#d4e4f4",fontFamily:"'Share Tech Mono',monospace",lineHeight:1.5}}>{item.title}</div><div style={{display:"flex",gap:8,marginTop:5,alignItems:"center",flexWrap:"wrap"}}><span style={{fontSize:7.5,color:"#22c55e"}}>{item.source}</span><span style={{fontSize:7.5,color:"#333"}}>{item.time}</span>{item.url&&<a href={item.url} target="_blank" rel="noreferrer" style={{fontSize:7.5,color:"#38bdf8",textDecoration:"none"}}>read full article ↗</a>}</div></div>
      ...
      <div style={{padding:"16px"}}><div style={{fontSize:7,color:"#818cf8",letterSpacing:3,marginBottom:12,fontFamily:"'Share Tech Mono',monospace",display:"flex",alignItems:"center",gap:8}}><span style={{width:5,height:5,background:"#818cf8",display:"inline-block",animation:loading?"pulse 1s infinite":"none"}}/> WARROOM AI ANALYSIS</div>
        {loading?<div style={{display:"flex",flexDirection:"column",gap:6}}>{[100,85,70].map((w,i)=><div key={i} style={{height:8,background:"#111",borderRadius:2,width:`${w}%`,animation:"pulse 1.2s infinite",animationDelay:`${i*0.15}s`}}/>)}<div style={{fontSize:9,color:"#333",marginTop:8,fontFamily:"'Share Tech Mono',monospace"}}>Analyzing intelligence...</div></div>:<div style={{borderLeft:"3px solid #818cf8",paddingLeft:14}}><AITextBlock text={summary} color="#8aaccc" fontSize={11}/></div>}
      </div>
      {item.image&&<div style={{padding:"0 16px 16px"}}><img src={item.image} alt="" style={{width:"100%",maxHeight:180,objectFit:"cover",borderRadius:3,border:"1px solid #1c1c1c",opacity:0.8}} onError={e=>{(e.target as HTMLImageElement).style.display="none";}}/></div>}
    </div>
  </div>;
}
...
function IntelPage({intel,news}:{intel:any;news:NewsItem[]}){
  ...
  if(isMobile){
    return <div style={{display:"flex",flexDirection:"column",gap:0}}>
      {/* Mobile tab switcher */}
      <div style={{display:"flex",borderBottom:"1px solid #111",background:"#050505",flexShrink:0,position:"sticky",top:0,zIndex:10}}>
        {[["intel","INTEL"],["news","NEWS"],["chat","AI CHAT"]].map(([id,label])=>(
          <button key={id} onClick={()=>setMobileView(id as any)} style={{flex:1,padding:"10px 4px",background:mobileView===id?"#0a0a0a":"transparent",border:"none",borderBottom:`2px solid ${mobileView===id?"#818cf8":"transparent"}`,color:mobileView===id?"#818cf8":"#444",fontSize:9,fontFamily:"'Share Tech Mono',monospace",letterSpacing:1,cursor:"pointer"}}>
            {label}
          </button>
        ))}
      </div>
      ...
      {mobileView==="intel"&&<div style={{display:"flex",flexDirection:"column",gap:10,padding:"10px"}}>
        <div style={{background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:4,padding:14}}>
          <div style={{fontSize:7,letterSpacing:4,color:"#333",marginBottom:10,display:"flex",justifyContent:"space-between",fontFamily:"'Share Tech Mono',monospace"}}><span>AI SITUATION BRIEF</span><span style={{color:"#22c55e"}}>CLAUDE</span></div>
        ...
        {intel.predictions&&<div style={{background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:4,overflow:"hidden"}}>
          <div style={{padding:"9px 13px",borderBottom:"1px solid #111",fontSize:7,letterSpacing:3,color:"#333",display:"flex",justifyContent:"space-between",alignItems:"center",fontFamily:"'Share Tech Mono',monospace"}}><span>ESCALATION FORECAST</span><div style={{display:"flex",gap:4,alignItems:"center"}}><div style={{width:4,height:4,background:"#ef4444",animation:"pulse 1.5s infinite"}}/><span style={{fontSize:7,color:"#ef4444",letterSpacing:2}}>AI</span></div></div>
          <div style={{padding:12,display:"flex",flexDirection:"column",gap:8}}>
            {intel.predictions.map((p:any,i:number)=>{const c=p.probability>80?"#ef4444":p.probability>65?"#f97316":"#eab308";return<div key={i} style={{padding:"11px 13px",borderRadius:3,background:"#080808",border:"1px solid #1c1c1c",borderLeft:`3px solid ${c}`}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}><span style={{fontSize:9,color:"#d4e4f4",fontWeight:700,fontFamily:"'Share Tech Mono',monospace"}}>{p.zone}</span><span style={{fontSize:14,fontWeight:900,color:c,fontFamily:"'Share Tech Mono',monospace"}}>{p.probability}%</span></div>
        ...
        <div style={{background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:4,padding:12}}>
          <div style={{fontSize:7,letterSpacing:3,color:"#333",marginBottom:10,fontFamily:"'Share Tech Mono',monospace"}}>CRITICAL EVENTS</div>
          {(intel.events||[]).filter((e:any)=>e.severity==="critical").map((ev:any,i:number)=><EventCard key={ev.id||i} ev={ev} isNew={i===0}/>)}
          {(intel.events||[]).filter((e:any)=>e.severity==="critical").length===0&&<div style={{color:"#333",fontSize:9,padding:20,textAlign:"center",letterSpacing:2,fontFamily:"'Share Tech Mono',monospace"}}>NO CRITICAL EVENTS</div>}
        </div>
      </div>}
      ...
      {mobileView==="news"&&<div style={{display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{padding:"8px 12px",borderBottom:"1px solid #111",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
          <div style={{display:"flex",gap:4}}>{([["latest","LATEST"],["24hrs","24H"],["hot","HOT"]] as [string,string][]).map(([id,label])=>(
            <button key={id} onClick={()=>setNewsTab(id as any)} style={{background:newsTab===id?"#111":"transparent",border:`1px solid ${newsTab===id?"#818cf8":"#1c1c1c"}`,borderRadius:3,color:newsTab===id?"#818cf8":"#444",padding:"3px 10px",cursor:"pointer",fontSize:8,fontFamily:"'Share Tech Mono',monospace",letterSpacing:1}}>{label}</button>
          ))}</div>
          <div style={{display:"flex",alignItems:"center",gap:4}}><span style={{fontSize:7.5,color:"#333"}}>{displayNews.length}</span><div style={{width:4,height:4,background:"#ef4444",animation:"pulse 1s infinite"}}/></div>
        </div>
        <div style={{padding:"6px 10px",borderBottom:"1px solid #0f0f0f"}}>
          <div style={{position:"relative"}}><span style={{position:"absolute",left:8,top:"50%",transform:"translateY(-50%)",fontSize:9,color:"#333"}}>⌕</span>
            <input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="Filter news..." style={{width:"100%",background:"#080808",border:"1px solid #1c1c1c",borderRadius:3,padding:"6px 8px 6px 22px",fontSize:10,color:"#8aaccc",fontFamily:"'Share Tech Mono',monospace",outline:"none"}}/>
            {searchQuery&&<button onClick={()=>setSearchQuery("")} style={{position:"absolute",right:6,top:"50%",transform:"translateY(-50%)",background:"transparent",border:"none",color:"#444",cursor:"pointer",fontSize:14,lineHeight:1}}>×</button>}
          </div>
        </div>
      ...
      {mobileView==="chat"&&<div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 160px)"}}>
        <div style={{padding:"8px 12px",borderBottom:"1px solid #1c1c1c",flexShrink:0,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div><div style={{fontSize:7,color:"#818cf8",letterSpacing:3,marginBottom:2,fontFamily:"'Share Tech Mono',monospace"}}>WARROOM AI CHATBOT</div><div style={{fontSize:8,color:"#333"}}>Ask about any conflict</div></div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"10px 12px",display:"flex",flexDirection:"column",gap:8}}>
          {chatMessages.length===0&&<div style={{padding:"8px 0"}}><div style={{fontSize:9,color:"#444",marginBottom:10,fontFamily:"'Share Tech Mono',monospace",lineHeight:1.6}}>WARROOM AI ready. Ask me anything.</div><div style={{display:"flex",flexWrap:"wrap",gap:5}}>{QUICK_PROMPTS.map((p,i)=><button key={i} onClick={()=>sendChat(p)} style={{background:"#0f0f0f",border:"1px solid #1c1c1c",color:"#444",padding:"5px 9px",borderRadius:3,cursor:"pointer",fontSize:8,fontFamily:"'Share Tech Mono',monospace",textAlign:"left"}}>{p}</button>)}</div></div>}
          {chatMessages.map(m=><div key={m.id} style={{display:"flex",flexDirection:"column",alignItems:m.role==="user"?"flex-end":"flex-start"}}><div style={{maxWidth:"92%",padding:"10px 13px",borderRadius:3,background:m.role==="user"?"#111":"#080808",border:`1px solid ${m.role==="user"?"#333":"#1c1c1c"}`,borderLeft:m.role==="assistant"?"3px solid #818cf8":"1px solid #333"}}>{m.role==="assistant"&&<div style={{fontSize:7,color:"#818cf8",letterSpacing:2,marginBottom:8,fontFamily:"'Share Tech Mono',monospace"}}>WARROOM AI</div>}{m.role==="user"?<div style={{fontSize:10,color:"#d4e4f4",fontFamily:"'Share Tech Mono',monospace",lineHeight:1.7}}>{m.content}</div>:<AITextBlock text={m.content} color="#8aaccc" fontSize={10}/>}</div></div>)}
        ...
        <div style={{padding:"8px 10px",borderTop:"1px solid #1c1c1c",flexShrink:0}}>
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
            <input value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendChat();}}} placeholder="Ask anything..." disabled={chatLoading} style={{flex:1,background:"#080808",border:"1px solid #222",borderRadius:3,padding:"9px 10px",fontSize:11,color:"#8aaccc",fontFamily:"'Share Tech Mono',monospace",outline:"none"}} onFocus={e=>e.target.style.borderColor="#818cf8"} onBlur={e=>e.target.style.borderColor="#222"}/>
            <button onClick={()=>sendChat()} disabled={chatLoading||!chatInput.trim()} style={{background:chatLoading||!chatInput.trim()?"transparent":"#818cf8",border:`1px solid ${chatLoading||!chatInput.trim()?"#222":"#818cf8"}`,color:chatLoading||!chatInput.trim()?"#333":"#000",padding:"9px 14px",borderRadius:3,cursor:chatLoading||!chatInput.trim()?"not-allowed":"pointer",fontSize:9,fontFamily:"'Share Tech Mono',monospace",fontWeight:700,letterSpacing:1,flexShrink:0}}>SEND</button>
          </div>
  ...
  return <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,height:"calc(100vh - 120px)"}}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:10,overflow:"hidden"}}>
      <div style={{background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:4,overflow:"hidden",display:"flex",flexDirection:"column",flex:"0 0 auto",maxHeight:"50%"}}>
        <div style={{padding:"8px 12px",borderBottom:"1px solid #111",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
          <div style={{display:"flex",gap:0}}>{([["latest","LATEST"],["24hrs","24 HRS"],["hot","HOT"]] as [string,string][]).map(([id,label])=><button key={id} onClick={()=>setNewsTab(id as any)} style={{background:newsTab===id?"#111":"transparent",border:`1px solid ${newsTab===id?"#818cf8":"#1c1c1c"}`,borderRadius:3,color:newsTab===id?"#818cf8":"#444",padding:"3px 12px",cursor:"pointer",fontSize:8,fontFamily:"'Share Tech Mono',monospace",letterSpacing:1,marginRight:4}}>{label}</button>)}</div>
          <div style={{display:"flex",alignItems:"center",gap:4}}><span style={{fontSize:7.5,color:"#333"}}>{displayNews.length} articles</span><div style={{width:4,height:4,background:"#ef4444",animation:"pulse 1s infinite"}}/></div>
        </div>
        <div style={{padding:"6px 10px",borderBottom:"1px solid #0f0f0f",flexShrink:0}}>
          <div style={{position:"relative"}}><span style={{position:"absolute",left:8,top:"50%",transform:"translateY(-50%)",fontSize:9,color:"#333",fontFamily:"'Share Tech Mono',monospace"}}>⌕</span>
            <input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="Filter news..." style={{width:"100%",background:"#080808",border:"1px solid #1c1c1c",borderRadius:3,padding:"5px 8px 5px 22px",fontSize:9,color:"#8aaccc",fontFamily:"'Share Tech Mono',monospace",outline:"none"}}/>
      ...
      <div style={{background:"#0a0a0a",border:"1px solid #818cf8",borderRadius:4,display:"flex",flexDirection:"column",flex:1,overflow:"hidden",minHeight:0}}>
        <div style={{padding:"8px 12px",borderBottom:"1px solid #1c1c1c",flexShrink:0,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div><div style={{fontSize:7,color:"#818cf8",letterSpacing:3,marginBottom:2,fontFamily:"'Share Tech Mono',monospace"}}>WARROOM AI — INTELLIGENCE CHATBOT</div><div style={{fontSize:8,color:"#333",fontFamily:"'Share Tech Mono',monospace"}}>Ask about conflicts, news, or get analysis</div></div>
          <div style={{display:"flex",gap:4,alignItems:"center"}}><div style={{width:5,height:5,background:"#818cf8",animation:"pulse 1.5s infinite"}}/><span style={{fontSize:7,color:"#818cf8",letterSpacing:2}}>ONLINE</span></div>
        </div>
        <div style={{flex:1,overflowY:"auto",padding:"10px 12px",display:"flex",flexDirection:"column",gap:8,minHeight:0}}>
          {chatMessages.length===0&&<div style={{padding:"8px 0"}}><div style={{fontSize:9,color:"#444",marginBottom:10,fontFamily:"'Share Tech Mono',monospace",lineHeight:1.6}}>WARROOM AI ready. I have context on current global conflicts and live news. Ask me anything.</div><div style={{display:"flex",flexWrap:"wrap",gap:5}}>{QUICK_PROMPTS.map((p,i)=><button key={i} onClick={()=>sendChat(p)} style={{background:"#0f0f0f",border:"1px solid #1c1c1c",color:"#444",padding:"4px 9px",borderRadius:3,cursor:"pointer",fontSize:8,fontFamily:"'Share Tech Mono',monospace",textAlign:"left"}} onMouseEnter={e=>{e.currentTarget.style.borderColor="#818cf8";e.currentTarget.style.color="#818cf8";}} onMouseLeave={e=>{e.currentTarget.style.borderColor="#1c1c1c";e.currentTarget.style.color="#444";}}>{p}</button>)}</div></div>}
          {chatMessages.map(m=><div key={m.id} style={{display:"flex",flexDirection:"column",alignItems:m.role==="user"?"flex-end":"flex-start"}}><div style={{maxWidth:"88%",padding:"10px 13px",borderRadius:3,background:m.role==="user"?"#111":"#080808",border:`1px solid ${m.role==="user"?"#333":"#1c1c1c"}`,borderLeft:m.role==="assistant"?"3px solid #818cf8":"1px solid #333"}}>{m.role==="assistant"&&<div style={{fontSize:7,color:"#818cf8",letterSpacing:2,marginBottom:8,fontFamily:"'Share Tech Mono',monospace"}}>WARROOM AI</div>}{m.role==="user"?<div style={{fontSize:10,color:"#d4e4f4",fontFamily:"'Share Tech Mono',monospace",lineHeight:1.7}}>{m.content}</div>:<AITextBlock text={m.content} color="#8aaccc" fontSize={10}/>}</div></div>)}
          {chatLoading&&<div style={{display:"flex",alignItems:"center",gap:6,padding:"6px 0"}}><div style={{width:5,height:5,background:"#818cf8",animation:"pulse 0.6s infinite"}}/><div style={{width:5,height:5,background:"#818cf8",animation:"pulse 0.6s infinite",animationDelay:"0.2s"}}/><div style={{width:5,height:5,background:"#818cf8",animation:"pulse 0.6s infinite",animationDelay:"0.4s"}}/><span style={{fontSize:8,color:"#444",fontFamily:"'Share Tech Mono',monospace"}}>Analyzing...</span></div>}
          <div ref={chatEndRef}/>
        </div>
        <div style={{padding:"8px 10px",borderTop:"1px solid #1c1c1c",flexShrink:0}}>
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
            <input value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendChat();}}} placeholder="Ask about any conflict or news..." disabled={chatLoading} style={{flex:1,background:"#080808",border:"1px solid #222",borderRadius:3,padding:"7px 10px",fontSize:10,color:"#8aaccc",fontFamily:"'Share Tech Mono',monospace",outline:"none"}} onFocus={e=>e.target.style.borderColor="#818cf8"} onBlur={e=>e.target.style.borderColor="#222"}/>
            <button onClick={()=>sendChat()} disabled={chatLoading||!chatInput.trim()} style={{background:chatLoading||!chatInput.trim()?"transparent":"#818cf8",border:`1px solid ${chatLoading||!chatInput.trim()?"#222":"#818cf8"}`,color:chatLoading||!chatInput.trim()?"#333":"#000",padding:"7px 14px",borderRadius:3,cursor:chatLoading||!chatInput.trim()?"not-allowed":"pointer",fontSize:9,fontFamily:"'Share Tech Mono',monospace",fontWeight:700,letterSpacing:1}}>SEND</button>
          </div>
          {chatMessages.length>0&&<button onClick={()=>setChatMessages([])} style={{marginTop:5,background:"transparent",border:"none",color:"#333",cursor:"pointer",fontSize:7.5,fontFamily:"'Share Tech Mono',monospace",letterSpacing:1}}>CLEAR CHAT</button>}
        </div>
      </div>
...
// ===== MarketsPage =====
function MarketsPage({crypto,stocks,cryptoCharts,stockCharts,loadingMarkets,selectedCoin,setSelectedCoin}:{crypto:CryptoCoin[];stocks:Stock[];cryptoCharts:Record<string,any[]>;stockCharts:Record<string,any[]>;loadingMarkets:boolean;selectedCoin:string;setSelectedCoin:(id:string)=>void}){
  const isMobile=useIsMobile();
  ...
  if(isMobile){
    return <div style={{display:"flex",flexDirection:"column"}}>
      <div style={{display:"flex",borderBottom:"1px solid #111",background:"#050505",flexShrink:0}}>
        {[["crypto","CRYPTO"],["chart","CHART"],["stocks","MARKETS"]].map(([id,label])=>(
          <button key={id} onClick={()=>setMobileTab(id as any)} style={{flex:1,padding:"10px 4px",background:mobileTab===id?"#0a0a0a":"transparent",border:"none",borderBottom:`2px solid ${mobileTab===id?"#22c55e":"transparent"}`,color:mobileTab===id?"#22c55e":"#444",fontSize:9,fontFamily:"'Share Tech Mono',monospace",letterSpacing:1,cursor:"pointer"}}>{label}</button>
        ))}
      </div>
      {mobileTab==="crypto"&&<div style={{padding:10}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {crypto.map(c=><CryptoCard key={c.id} coin={c} chart={cryptoCharts[c.id]||[]} selected={selectedCoin===c.id} onClick={()=>{setSelectedCoin(c.id);setMobileTab("chart");}}/>)}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,marginTop:12}}>
          {[{label:"BTC DOM",value:`${((crypto[0]?.marketCap/(crypto.reduce((a,c)=>a+(c.marketCap||0),0)||1))*100).toFixed(1)}%`,color:"#f97316"},{label:"TOTAL MCAP",value:fmtB(crypto.reduce((a,c)=>a+(c.marketCap||0),0)),color:"#22c55e"},{label:"24H VOL",value:fmtB(crypto.reduce((a,c)=>a+(c.volume||0),0)),color:"#818cf8"},{label:"MOOD",value:crypto.filter(c=>c.change>0).length>crypto.length/2?"BULLISH":"BEARISH",color:crypto.filter(c=>c.change>0).length>crypto.length/2?"#22c55e":"#ef4444"}].map((s,i)=>(
            <div key={i} style={{padding:"10px 12px",borderRadius:4,background:"#0a0a0a",border:"1px solid #1c1c1c",borderTop:`2px solid ${s.color}`}}><div style={{fontSize:7,color:"#333",letterSpacing:2,marginBottom:5,fontFamily:"'Share Tech Mono',monospace"}}>{s.label}</div><div style={{fontSize:14,fontWeight:900,color:s.color,fontFamily:"'Share Tech Mono',monospace"}}>{s.value}</div></div>
          ))}
        </div>
      </div>}
      {mobileTab==="chart"&&coin&&<div style={{padding:10}}>
        <div style={{background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:4,padding:"14px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:36,height:36,borderRadius:3,background:"#111",border:`1px solid ${coin.color}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,color:coin.color,fontWeight:900,fontFamily:"'Share Tech Mono',monospace"}}>{coin.symbol}</div><div><div style={{fontSize:14,fontWeight:900,color:"#e2e8f0",fontFamily:"'Share Tech Mono',monospace"}}>{coin.name}</div><div style={{fontSize:9,color:"#333"}}>{coin.id} / USD</div></div></div>
            <div style={{textAlign:"right"}}><div style={{fontSize:20,fontWeight:900,color:"#e2e8f0",fontFamily:"'Share Tech Mono',monospace"}}>${fmt(coin.price,coin.price>100?0:2)}</div><div style={{fontSize:10,color:isUp?"#22c55e":"#ef4444"}}>{isUp?"+":"-"}{Math.abs(coin.change).toFixed(2)}%</div></div>
          </div>
          <div style={{display:"flex",gap:6,marginBottom:12}}>{[{v:1,l:"1D"},{v:7,l:"7D"},{v:30,l:"1M"}].map(r=><button key={r.v} onClick={()=>setChartRange(r.v)} style={{background:chartRange===r.v?"#111":"transparent",border:`1px solid ${chartRange===r.v?coin.color:"#1c1c1c"}`,color:chartRange===r.v?coin.color:"#333",padding:"3px 14px",borderRadius:3,cursor:"pointer",fontSize:9,fontFamily:"'Share Tech Mono',monospace"}}>{r.l}</button>)}</div>
          <div style={{height:200}}><ResponsiveContainer width="100%" height="100%"><AreaChart data={chart} margin={{top:5,right:5,bottom:0,left:0}}><defs><linearGradient id="coinGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={coin.color} stopOpacity={0.12}/><stop offset="95%" stopColor={coin.color} stopOpacity={0}/></linearGradient></defs><XAxis dataKey="time" tick={{fontSize:7,fill:"#333"}} axisLine={false} tickLine={false} interval="preserveStartEnd"/><YAxis tick={{fontSize:7,fill:"#333"}} axisLine={false} tickLine={false} width={55} tickFormatter={v=>`$${v>=1000?(v/1000).toFixed(1)+"k":v}`} domain={["auto","auto"]}/><Tooltip content={<ChartTip/>}/><Area type="monotone" dataKey="price" stroke={coin.color} strokeWidth={1.5} fill="url(#coinGrad)" dot={false} activeDot={{r:4,fill:coin.color}}/></AreaChart></ResponsiveContainer></div>
        </div>
      </div>}
      {mobileTab==="stocks"&&<div style={{padding:10}}>
        <div style={{background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:4,overflow:"hidden",marginBottom:12}}>
          <div style={{padding:"9px 13px",borderBottom:"1px solid #111",fontSize:7,letterSpacing:3,color:"#333",fontFamily:"'Share Tech Mono',monospace",display:"flex",justifyContent:"space-between"}}><span>INDICES & COMMODITIES</span><span style={{color:"#22c55e"}}>LIVE</span></div>
  ...
  return <div style={{display:"grid",gridTemplateColumns:"1fr 280px",gap:14}}>
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      <div><div style={{fontSize:7,letterSpacing:4,color:"#333",marginBottom:8,fontFamily:"'Share Tech Mono',monospace",display:"flex",alignItems:"center",gap:8}}><span style={{width:5,height:5,background:"#22c55e",display:"inline-block"}}/>LIVE CRYPTO — COINGECKO{loadingMarkets&&<span style={{color:"#333"}}>syncing...</span>}</div><div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8}}>{crypto.map(c=><CryptoCard key={c.id} coin={c} chart={cryptoCharts[c.id]||[]} selected={selectedCoin===c.id} onClick={()=>setSelectedCoin(c.id)}/>)}</div></div>
      {coin&&<div style={{background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:4,padding:"14px 16px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:36,height:36,borderRadius:3,background:"#111",border:`1px solid ${coin.color}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,color:coin.color,fontWeight:900,fontFamily:"'Share Tech Mono',monospace"}}>{coin.symbol}</div><div><div style={{fontSize:14,fontWeight:900,color:"#e2e8f0",fontFamily:"'Share Tech Mono',monospace"}}>{coin.name}</div><div style={{fontSize:9,color:"#333"}}>{coin.id} / USD</div></div></div><div style={{textAlign:"right"}}><div style={{fontSize:22,fontWeight:900,color:"#e2e8f0",fontFamily:"'Share Tech Mono',monospace"}}>${fmt(coin.price,coin.price>100?0:2)}</div><div style={{fontSize:11,color:isUp?"#22c55e":"#ef4444"}}>{isUp?"+":"-"}{Math.abs(coin.change).toFixed(2)}% (24h)</div></div></div>
      ...
      <div style={{background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:4,padding:"14px 16px"}}>
        <div style={{fontSize:7,letterSpacing:4,color:"#333",marginBottom:12,fontFamily:"'Share Tech Mono',monospace",display:"flex",alignItems:"center",gap:8}}><span style={{width:5,height:5,background:"#818cf8",display:"inline-block"}}/>GLOBAL MARKETS — LIVE</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>{["SPY","QQQ","GC=F","CL=F"].map(sym=>{const s=stocks.find(st=>st.id===sym);const ch=stockCharts[sym]||[];if(!s)return null;const up=s.change>=0;return<div key={sym} style={{background:"#080808",border:"1px solid #1c1c1c",borderRadius:3,padding:"10px 12px"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><div><div style={{fontSize:10,color:"#c4d4e8",fontFamily:"'Share Tech Mono',monospace",fontWeight:700}}>{s.id}</div><div style={{fontSize:7.5,color:"#333"}}>{s.name}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:11,color:"#e2e8f0",fontFamily:"'Share Tech Mono',monospace",fontWeight:700}}>{s.price>0?`$${fmt(s.price,s.price>500?0:2)}`:"—"}</div><div style={{fontSize:9,color:up?"#22c55e":"#ef4444"}}>{up?"+":"-"}{Math.abs(s.change).toFixed(2)}%</div></div></div><Sparkline data={ch} color={s.color} height={50}/></div>;})}
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>{[{label:"BTC DOMINANCE",value:`${((crypto[0]?.marketCap/(crypto.reduce((a,c)=>a+(c.marketCap||0),0)||1))*100).toFixed(1)}%`,color:"#f97316"},{label:"TOTAL MCap",value:fmtB(crypto.reduce((a,c)=>a+(c.marketCap||0),0)),color:"#22c55e"},{label:"24H VOLUME",value:fmtB(crypto.reduce((a,c)=>a+(c.volume||0),0)),color:"#818cf8"},{label:"MARKET MOOD",value:crypto.filter(c=>c.change>0).length>crypto.length/2?"BULLISH":"BEARISH",color:crypto.filter(c=>c.change>0).length>crypto.length/2?"#22c55e":"#ef4444"}].map((s,i)=><div key={i} style={{padding:"11px 13px",borderRadius:4,background:"#0a0a0a",border:"1px solid #1c1c1c",borderTop:`2px solid ${s.color}`}}><div style={{fontSize:7,color:"#333",letterSpacing:2,marginBottom:6,fontFamily:"'Share Tech Mono',monospace"}}>{s.label}</div><div style={{fontSize:15,fontWeight:900,color:s.color,fontFamily:"'Share Tech Mono',monospace"}}>{s.value}</div></div>)}</div>
    </div>
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      <div style={{background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:4,overflow:"hidden"}}><div style={{padding:"9px 13px",borderBottom:"1px solid #111",fontSize:7,letterSpacing:3,color:"#333",fontFamily:"'Share Tech Mono',monospace",display:"flex",justifyContent:"space-between",alignItems:"center"}}><span>INDICES & COMMODITIES</span><span style={{fontSize:7,color:"#22c55e"}}>LIVE</span></div>{stocks.map(s=><StockRow key={s.id} s={s}/>)}</div>
      <div style={{background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:4,overflow:"hidden"}}><div style={{padding:"9px 13px",borderBottom:"1px solid #111",fontSize:7,letterSpacing:3,color:"#333",fontFamily:"'Share Tech Mono',monospace"}}>TOP CRYPTO</div>{crypto.map(c=>{const up=c.change>=0;return<div key={c.id} onClick={()=>setSelectedCoin(c.id)} style={{display:"flex",alignItems:"center",padding:"8px 13px",borderBottom:"1px solid #0f0f0f",cursor:"pointer",transition:"background .12s"}} onMouseEnter={e=>e.currentTarget.style.background="#0f0f0f"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}><div style={{width:22,height:22,borderRadius:2,background:"#111",border:`1px solid ${c.color}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:7,color:c.color,fontWeight:900,marginRight:9,flexShrink:0,fontFamily:"'Share Tech Mono',monospace"}}>{c.symbol}</div><div style={{flex:1}}><div style={{fontSize:10,color:"#c4d4e8",fontFamily:"'Share Tech Mono',monospace"}}>{c.id}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:10,color:"#e2e8f0",fontFamily:"'Share Tech Mono',monospace"}}>${fmt(c.price,c.price>100?0:2)}</div><div style={{fontSize:8,color:up?"#22c55e":"#ef4444"}}>{up?"+":"-"}{Math.abs(c.change).toFixed(2)}%</div></div></div>;})}</div>
      <div style={{padding:"12px 14px",borderRadius:4,background:"#0a0a0a",border:"1px solid #1c1c1c",fontSize:8,color:"#333",lineHeight:1.7}}><div style={{color:"#444",marginBottom:4,letterSpacing:2,fontSize:7}}>DATA SOURCES</div>Crypto: CoinGecko API (real-time)<br/>Stocks: Yahoo Finance (15min delay)<br/>Auto-refresh every 60 seconds</div>
    </div>
  </div>;
}
...
function VideosPage({videos,news,loadingVideos,playingVideo,setPlayingVideo}:{videos:VideoItem[];news:NewsItem[];loadingVideos:boolean;playingVideo:VideoItem|null;setPlayingVideo:(v:VideoItem|null)=>void}){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:12}}>
    <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}><span style={{fontSize:7,color:"#333",letterSpacing:3,fontFamily:"'Share Tech Mono',monospace"}}>TOPIC:</span>{topics.map(t=><button key={t} onClick={()=>setTopic(t)} style={{background:topic===t?"#111":"transparent",border:`1px solid ${topic===t?"#ef4444":"#1c1c1c"}`,color:topic===t?"#ef4444":"#444",padding:"3px 10px",borderRadius:3,cursor:"pointer",fontSize:8,fontFamily:"'Share Tech Mono',monospace",textTransform:"uppercase"}}>{t}</button>)}</div>
    {playingVideo&&<div style={{background:"#080808",border:"1px solid #1c1c1c",borderRadius:4,overflow:"hidden"}}><div style={{position:"relative",paddingBottom:"56.25%"}}><iframe src={`https://www.youtube.com/embed/${playingVideo.videoId}?autoplay=1&rel=0`} style={{position:"absolute",inset:0,width:"100%",height:"100%",border:"none"}} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen/></div><div style={{padding:"10px 14px",display:"flex",gap:10,alignItems:"center"}}><div style={{flex:1}}><div style={{fontSize:11,color:"#d4e4f4",fontFamily:"'Share Tech Mono',monospace",marginBottom:3}}>{playingVideo.title}</div><div style={{fontSize:8,color:"#444"}}>{playingVideo.channel}</div></div><a href={playingVideo.url} target="_blank" rel="noreferrer" style={{fontSize:8,color:"#38bdf8",textDecoration:"none",border:"1px solid #38bdf822",padding:"4px 12px",borderRadius:3,fontFamily:"'Share Tech Mono',monospace",flexShrink:0}}>YOUTUBE</a><button onClick={()=>setPlayingVideo(null)} style={{background:"transparent",border:"1px solid #1c1c1c",color:"#444",padding:"4px 10px",borderRadius:3,cursor:"pointer",fontSize:8,fontFamily:"'Share Tech Mono',monospace"}}>CLOSE</button></div></div>}
    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 320px",gap:14}}>
      <div>
        <div style={{fontSize:7,letterSpacing:4,color:"#333",marginBottom:8,fontFamily:"'Share Tech Mono',monospace",display:"flex",alignItems:"center",gap:8}}><span style={{width:5,height:5,background:"#ef4444",display:"inline-block"}}/>LIVE NEWS VIDEOS{loadingVideos&&<span style={{color:"#333"}}>loading...</span>}</div>
      ...
      {!isMobile&&<div style={{background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:4,overflow:"hidden",flex:1}}><div style={{padding:"9px 13px",borderBottom:"1px solid #111",fontSize:7,letterSpacing:3,color:"#333",fontFamily:"'Share Tech Mono',monospace",display:"flex",justifyContent:"space-between"}}><span>LATEST NEWS</span><span style={{color:"#ef4444"}}>LIVE</span></div><div style={{padding:10,maxHeight:600,overflowY:"auto"}}>{news.map((item,i)=><NewsCard key={i} item={item} big={i<3}/>)}{news.length===0&&<div style={{padding:24,textAlign:"center",color:"#333",fontSize:9}}>Loading headlines...</div>}</div></div>}
    </div>
...
function TrendingPage({tab,news,videos,setPlayingVideo}:{tab:any;news:NewsItem[];videos:VideoItem[];setPlayingVideo:(v:VideoItem|null)=>void}){
  ...
  const topicNews=news.filter(n=>{const t=(n.title||"").toLowerCase();return tab.keywords?.some((k:string)=>t.includes(k));});
  const topicVideos=videos.filter(v=>{const t=(v.title||"").toLowerCase();return tab.keywords?.some((k:string)=>t.includes(k));});
  return <div style={{display:"flex",flexDirection:"column",gap:12}}>
    <div style={{padding:isMobile?"12px":"16px 20px",borderRadius:4,background:"#0a0a0a",border:`1px solid ${tab.color}`,borderLeft:`4px solid ${tab.color}`,display:"flex",alignItems:"center",gap:16}}>
      ...
      <div style={{flex:1}}>
        ...
        <div style={{fontSize:isMobile?14:18,fontWeight:900,color:"#e2e8f0",fontFamily:"'Share Tech Mono',monospace",letterSpacing:2}}>[{tab.flag}] {tab.label}</div>
        <div style={{fontSize:8,color:"#444",marginTop:4}}>{tab.eventCount} events · {topicNews.length} articles · expires in {timeLeft}h</div>
      </div>
      <div style={{textAlign:"center",flexShrink:0}}>
        <div style={{fontSize:7,color:"#333",letterSpacing:2,marginBottom:6}}>HEAT</div>
        <div style={{position:"relative",width:52,height:52}}><svg viewBox="0 0 52 52" style={{transform:"rotate(-90deg)"}}><circle cx="26" cy="26" r="21" fill="none" stroke="#1c1c1c" strokeWidth="5"/><circle cx="26" cy="26" r="21" fill="none" stroke={tab.color} strokeWidth="5" strokeDasharray={`${(tab.heat/100)*131.9} 131.9`} strokeLinecap="round"/></svg><div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:900,color:tab.color,fontFamily:"'Share Tech Mono',monospace"}}>{tab.heat}</div></div>
      </div>
    </div>
    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{[["feed","INTEL"],["news","NEWS"],["videos","VIDEOS"],["map","MAP"]].map(([id,l])=><button key={id} onClick={()=>setSub(id)} style={{background:sub===id?"#111":"transparent",border:`1px solid ${sub===id?tab.color:"#1c1c1c"}`,color:sub===id?tab.color:"#444",padding:"5px 14px",borderRadius:3,cursor:"pointer",fontSize:9,fontFamily:"'Share Tech Mono',monospace",letterSpacing:1}}>{l}</button>)}</div>
    {sub==="feed"&&<div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:10}}><div style={{background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:4,padding:12}}><div style={{fontSize:7,letterSpacing:3,color:"#333",marginBottom:10,fontFamily:"'Share Tech Mono',monospace"}}>CONFLICT EVENTS</div>{tab.events?.map((ev:any,i:number)=><EventCard key={ev.id||i} ev={ev} isNew={i<2}/>)}</div><div style={{background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:4,padding:12}}><div style={{fontSize:7,letterSpacing:3,color:"#333",marginBottom:10,fontFamily:"'Share Tech Mono',monospace"}}>SEVERITY BREAKDOWN</div>{["critical","high","medium","low"].map(s=>{const n=tab.events?.filter((e:any)=>e.severity===s).length||0;if(!n)return null;return<div key={s} style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:9,color:SEV[s],textTransform:"uppercase",letterSpacing:1.5}}>{s}</span><span style={{fontSize:9,color:"#444"}}>{n}</span></div><div style={{height:3,background:"#111",borderRadius:1}}><div style={{height:"100%",width:`${(n/(tab.events?.length||1))*100}%`,background:SEV[s],borderRadius:1,transition:"width 1s"}}/></div></div>;})}</div></div>}
    {sub==="news"&&<div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:8}}>{topicNews.length===0?<div style={{padding:32,textAlign:"center",color:"#333",fontSize:10}}>No news matching this topic</div>:topicNews.map((n,i)=><NewsCard key={i} item={n} big/>)}</div>}
    {sub==="videos"&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:10}}>{topicVideos.length===0?<div style={{padding:32,textAlign:"center",color:"#333",fontSize:10,gridColumn:"1/-1"}}>No videos yet</div>:topicVideos.map((v,i)=><VideoCard key={v.videoId||i} v={v} onPlay={setPlayingVideo} playing={false}/>)}</div>}
    {sub==="map"&&<div style={{background:"#050505",border:"1px solid #1c1c1c",borderRadius:4,overflow:"hidden",height:isMobile?220:380}}><WorldMap hotspots={HOTSPOTS} events={tab.events||[]} selectedSpot={null} onSelect={()=>{}}/></div>}
  </div>;
}
...
function MobileBottomNav({page,setPage,trendingTabs}:{page:string;setPage:(p:string)=>void;trendingTabs:any[]}){
  ...
  return <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:200,background:"#050505",borderTop:"1px solid #111",display:"flex",height:56,paddingBottom:"env(safe-area-inset-bottom)"}}>
    {navItems.map(n=>{const isOn=page===n.id;return<button key={n.id} onClick={()=>setPage(n.id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,background:"transparent",border:"none",borderTop:`2px solid ${isOn?"#ef4444":"transparent"}`,color:isOn?"#ef4444":"#333",cursor:"pointer",padding:"6px 0"}}>
      <span style={{fontSize:14,fontFamily:"'Share Tech Mono',monospace",fontWeight:900,lineHeight:1}}>{n.icon}</span>
      <span style={{fontSize:7,fontFamily:"'Share Tech Mono',monospace",letterSpacing:0.5}}>{n.label.split(" ")[0]}</span>
    </button>;})}
  </div>;
}
...
export default function WarRoom(){
  ...
  useEffect(()=>{const t=setTimeout(()=>setTrendingTabs(p=>p.map(tt=>({...tt,isNew:false}))),8000);return()=>clearTimeout(t);},[trendingTabs.map(t=>t.id).join(",")]);
  ...
  if(loading)return(<div style={{position:"fixed",inset:0,background:"#000",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Share Tech Mono',monospace",padding:20}}>
    <div style={{fontSize:7,letterSpacing:8,color:"#333",marginBottom:10,textAlign:"center"}}>GLOBAL INTELLIGENCE PLATFORM</div>
    <div style={{fontSize:isMobile?28:38,fontWeight:900,letterSpacing:8,color:"#e2e8f0",marginBottom:6}}>WAR<span style={{color:"#ef4444"}}>ROOM</span></div>
    <div style={{width:"min(260px,80vw)",height:1,background:"#1c1c1c",overflow:"hidden",marginBottom:20,marginTop:30}}><div style={{height:"100%",width:"85%",background:"#ef4444",animation:"loadBar 2s ease infinite"}}/></div>
  ...
  return(<div style={{background:"#000",minHeight:"100vh",color:"#e2e8f0",fontFamily:"'Share Tech Mono','Courier New',monospace",display:"flex",flexDirection:"column",fontSize:12}}>
    <style>{`
    ...
    {alert&&<div style={{position:"fixed",top:0,left:0,right:0,zIndex:9999,background:"#ef4444",padding:"7px 16px",display:"flex",alignItems:"center",gap:10,animation:"fadeUp .2s ease"}}>
      ...
      <span style={{fontSize:isMobile?9:10,color:"#fff",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{alert.headline}</span>
    ...
    <div style={{height:28,background:"#080808",borderBottom:"1px solid #111",overflow:"hidden",position:"relative",flexShrink:0}}>
      <div style={{display:"flex",animation:"ticker 40s linear infinite",whiteSpace:"nowrap",height:"100%",alignItems:"center"}}>
        {[...Array(2)].map((_,rep)=><span key={rep} style={{display:"flex",gap:0}}>
          {[...crypto,...stocks].filter(i=>i.price>0).map((item,i)=>{const up=item.change>=0;return<span key={`${rep}-${i}`} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"0 18px",borderRight:"1px solid #111",fontSize:9,fontFamily:"'Share Tech Mono',monospace"}}><span style={{color:"#444"}}>{item.id||item.name}</span><span style={{color:"#c4d4e8",fontWeight:700}}>${fmt(item.price,item.price>100?0:2)}</span><span style={{color:up?"#22c55e":"#ef4444",fontSize:8}}>{up?"+":"-"}{Math.abs(item.change).toFixed(2)}%</span></span>;})}
          {intel&&<span style={{display:"inline-flex",alignItems:"center",gap:6,padding:"0 18px",borderRight:"1px solid #111",fontSize:9,color:"#ef4444"}}>[!] THREAT: {intel.globalThreatLevel}% | DEFCON {intel.defcon}</span>}
        </span>)}
      </div>
    </div>

    {/* Header */}
    <div style={{height:isMobile?46:50,background:"#050505",borderBottom:"1px solid #111",display:"flex",alignItems:"center",padding:`0 ${isMobile?12:16}px`,gap:isMobile?8:12,position:"sticky",top:28,zIndex:100,flexShrink:0}}>
      <button onClick={()=>isMobile?setMobileSidebarOpen(o=>!o):setSidebarOpen(o=>!o)} style={{background:"transparent",border:"none",color:"#444",cursor:"pointer",fontSize:16,padding:3,lineHeight:1,fontFamily:"monospace",flexShrink:0}}>&#9776;</button>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        ...
        <div style={{fontSize:isMobile?11:13,fontWeight:700,letterSpacing:isMobile?2:4,color:"#e2e8f0",fontFamily:"'Share Tech Mono',monospace",whiteSpace:"nowrap"}}>WAR<span style={{color:"#ef4444"}}>ROOM</span></div>
      </div>
      {!isMobile&&<><div style={{width:1,height:20,background:"#111"}}/><div style={{fontSize:7,letterSpacing:3,color:"#333",fontFamily:"'Share Tech Mono',monospace"}}>GLOBAL INTELLIGENCE PLATFORM</div></>}
      {btc&&!isMobile&&<div style={{display:"flex",gap:6,marginLeft:8}}>
        {[btc,eth,sol].filter(Boolean).map(c=>{if(!c)return null;return<div key={c.id} onClick={()=>{setPage("markets");setSelectedCoin(c.id);}} style={{display:"flex",alignItems:"center",gap:5,padding:"3px 9px",background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:3,cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.borderColor="#333"} onMouseLeave={e=>e.currentTarget.style.borderColor="#1c1c1c"}><span style={{fontSize:9,color:c.color,fontWeight:700,fontFamily:"'Share Tech Mono',monospace"}}>{c.id}</span><span style={{fontSize:9,color:"#c4d4e8",fontFamily:"'Share Tech Mono',monospace"}}>${fmt(c.price,c.price>100?0:2)}</span><span style={{fontSize:8,color:c.change>=0?"#22c55e":"#ef4444"}}>{c.change>=0?"+":"-"}{Math.abs(c.change).toFixed(1)}%</span></div>;})}
      </div>}
      <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:isMobile?6:8}}>
        {intel&&<div style={{display:"flex",alignItems:"center",gap:6,padding:"3px 9px",background:"#0a0a0a",border:"1px solid #ef4444",borderRadius:3}}>
        ...
        {!isMobile&&<button onClick={()=>loadIntel(undefined,true)} disabled={refreshingIntel} style={{background:"transparent",border:"1px solid #222",color:"#444",cursor:"pointer",fontSize:8,padding:"4px 10px",borderRadius:3,fontFamily:"'Share Tech Mono',monospace",letterSpacing:1}}>{refreshingIntel?"UPDATING...":"REFRESH"}</button>}
        {!isMobile&&lastUpdated&&<div style={{fontSize:8,color:"#333",fontFamily:"'Share Tech Mono',monospace"}}>{lastUpdated.toLocaleTimeString("en-US",{hour12:false})}</div>}
      </div>
    </div>
    ...
    {isMobile&&mobileSidebarOpen&&<div style={{position:"fixed",inset:0,zIndex:500,display:"flex"}} onClick={()=>setMobileSidebarOpen(false)}>
      <div style={{width:"min(280px,85vw)",background:"linear-gradient(180deg,#080808 0%,#040404 100%)",borderRight:"1px solid #111",height:"100%",overflowY:"auto",padding:"12px 0"}} onClick={e=>e.stopPropagation()}>
        <div style={{height:2,background:"linear-gradient(90deg,#ef4444 0%,#f97316 40%,#818cf8 75%,transparent 100%)",marginBottom:12}}/>
        {STATIC_NAV.map(n=>{const isOn=page===n.id;return<div key={n.id} onClick={()=>{setPage(n.id);setMobileSidebarOpen(false);}} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",cursor:"pointer",background:isOn?"rgba(239,68,68,0.07)":"transparent",borderLeft:`3px solid ${isOn?"#ef4444":"transparent"}`}}>
          <div style={{width:32,height:32,borderRadius:4,background:isOn?"rgba(239,68,68,0.14)":"#0d0d0d",border:`1px solid ${isOn?"rgba(239,68,68,0.35)":"#1a1a1a"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,color:isOn?"#ef4444":"#444",fontFamily:"'Share Tech Mono',monospace",fontWeight:900,flexShrink:0}}>{n.icon}</div>
          <span style={{fontSize:12,fontFamily:"'Share Tech Mono',monospace",color:isOn?"#d4e4f4":"#555",letterSpacing:0.5}}>{n.label}</span>
        </div>;})}
        {trendingTabs.length>0&&<><div style={{height:1,background:"linear-gradient(90deg,transparent,#151515,transparent)",margin:"8px 12px"}}/>
          <div style={{padding:"4px 18px 8px",fontSize:7,letterSpacing:5,color:"#1a1a1a",fontFamily:"'Share Tech Mono',monospace",display:"flex",alignItems:"center",gap:7}}><span style={{width:5,height:5,background:"#f97316",display:"inline-block",borderRadius:1,animation:"pulse 1.5s infinite"}}/>TRENDING</div>
          {trendingTabs.map(t=>{const tid=`trend:${t.id}`;const on=page===tid;return<div key={t.id} onClick={()=>{setPage(tid);setMobileSidebarOpen(false);}} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",cursor:"pointer",background:on?`${t.color}0d`:"transparent",borderLeft:`3px solid ${on?t.color:"transparent"}`}}>
            <div style={{width:32,height:32,borderRadius:4,background:on?`${t.color}18`:"#0d0d0d",border:`1px solid ${on?t.color+"44":"#1a1a1a"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:on?t.color:"#444",fontFamily:"'Share Tech Mono',monospace",fontWeight:900,flexShrink:0}}>{t.icon}</div>
            <div style={{flex:1,minWidth:0}}><div style={{fontSize:10,color:on?"#d4e4f4":"#555",fontFamily:"'Share Tech Mono',monospace",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>[{t.flag}] {t.label}</div><div style={{height:2,background:"#0f0f0f",borderRadius:1,marginTop:4}}><div style={{height:"100%",width:`${t.heat}%`,background:`linear-gradient(90deg,${t.color}60,${t.color})`,borderRadius:1}}/></div></div>
            {t.isNew&&<span style={{fontSize:6,background:t.color,color:"#000",padding:"2px 5px",borderRadius:2,fontWeight:900,flexShrink:0}}>NEW</span>}
          </div>;})}
        </>}
      </div>
    ...
    <div style={{display:"flex",flex:1,overflow:"hidden",height:`calc(100vh - ${isMobile?74:78}px)`}}>
      {/* Desktop Sidebar */}
      {!isMobile&&<div style={{width:sidebarOpen?220:50,minWidth:sidebarOpen?220:50,background:"linear-gradient(180deg,#050505 0%,#030303 100%)",borderRight:"1px solid #0f0f0f",display:"flex",flexDirection:"column",transition:"width .22s cubic-bezier(.4,0,.2,1), min-width .22s cubic-bezier(.4,0,.2,1)",overflow:"hidden",flexShrink:0}}>
        ...
        <div style={{flex:1,paddingTop:8,overflowY:"auto",overflowX:"hidden",scrollbarWidth:"none"}}>
          {sidebarOpen&&<div style={{padding:"6px 18px 5px",fontSize:7,letterSpacing:5,color:"#1a1a1a",fontFamily:"'Share Tech Mono',monospace"}}>NAV</div>}
          {STATIC_NAV.map(n=>{const isOn=page===n.id;return<div key={n.id} onClick={()=>setPage(n.id)} style={{display:"flex",alignItems:"center",gap:10,padding:sidebarOpen?"8px 12px 8px 14px":"9px 0",justifyContent:sidebarOpen?"flex-start":"center",cursor:"pointer",margin:"1px 6px",borderRadius:4,background:isOn?"rgba(239,68,68,0.07)":"transparent",borderLeft:`2px solid ${isOn?"#ef4444":"transparent"}`,transition:"all .15s ease"}} onMouseEnter={e=>{if(!isOn){e.currentTarget.style.background="rgba(255,255,255,0.025)";e.currentTarget.style.borderLeftColor="#252525";}}} onMouseLeave={e=>{if(!isOn){e.currentTarget.style.background="transparent";e.currentTarget.style.borderLeftColor="transparent";}}}>
            <div style={{width:28,height:28,borderRadius:4,background:isOn?"rgba(239,68,68,0.14)":"#080808",border:`1px solid ${isOn?"rgba(239,68,68,0.35)":"#161616"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:isOn?"#ef4444":"#333",fontFamily:"'Share Tech Mono',monospace",fontWeight:900,flexShrink:0,transition:"all .15s",boxShadow:isOn?"0 0 14px rgba(239,68,68,0.15)":"none"}}>{n.icon}</div>
            {sidebarOpen&&<><span style={{fontSize:10,fontFamily:"'Share Tech Mono',monospace",color:isOn?"#d4e4f4":"#484848",letterSpacing:0.3,flex:1,transition:"color .15s"}}>{n.label}</span>{isOn&&<div style={{width:3,height:16,borderRadius:2,background:"#ef4444",opacity:0.7,flexShrink:0}}/>}</>}
          </div>;})}
          {trendingTabs.length>0&&<>
            <div style={{height:1,background:"linear-gradient(90deg,transparent,#151515,transparent)",margin:"8px 12px"}}/>
            {sidebarOpen&&<div style={{padding:"4px 18px 6px",fontSize:7,letterSpacing:5,color:"#1a1a1a",fontFamily:"'Share Tech Mono',monospace",display:"flex",alignItems:"center",gap:7}}><span style={{width:5,height:5,background:"#f97316",display:"inline-block",borderRadius:1,animation:"pulse 1.5s infinite"}}/>TRENDING</div>}
            {trendingTabs.map(t=>{const tid=`trend:${t.id}`;const on=page===tid;return<div key={t.id} onClick={()=>setPage(tid)} style={{display:"flex",alignItems:"center",gap:9,padding:sidebarOpen?"8px 12px 8px 14px":"9px 0",justifyContent:sidebarOpen?"flex-start":"center",cursor:"pointer",margin:"1px 6px",borderRadius:4,background:on?`${t.color}0d`:"transparent",borderLeft:`2px solid ${on?t.color:"transparent"}`,transition:"all .15s ease"}} onMouseEnter={e=>{if(!on){e.currentTarget.style.background="rgba(255,255,255,0.025)";e.currentTarget.style.borderLeftColor="#252525";}}} onMouseLeave={e=>{if(!on){e.currentTarget.style.background="transparent";e.currentTarget.style.borderLeftColor="transparent";}}}>
              <div style={{width:28,height:28,borderRadius:4,background:on?`${t.color}18`:"#080808",border:`1px solid ${on?t.color+"44":"#161616"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,color:on?t.color:"#333",fontFamily:"'Share Tech Mono',monospace",fontWeight:900,flexShrink:0,transition:"all .15s",boxShadow:on?`0 0 14px ${t.color}20`:"none"}}>{t.icon}</div>
              {sidebarOpen&&<><div style={{flex:1,minWidth:0}}><div style={{fontSize:9,color:on?"#d4e4f4":"#484848",fontFamily:"'Share Tech Mono',monospace",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",transition:"color .15s"}}>[{t.flag}] {t.label}</div><div style={{height:2,background:"#0f0f0f",borderRadius:1,marginTop:4,overflow:"hidden"}}><div style={{height:"100%",width:`${t.heat}%`,background:`linear-gradient(90deg,${t.color}60,${t.color})`,borderRadius:1,transition:"width 1.2s cubic-bezier(.4,0,.2,1)"}}/></div></div>{t.isNew&&<span style={{fontSize:6,background:t.color,color:"#000",padding:"2px 5px",borderRadius:2,fontWeight:900,flexShrink:0,letterSpacing:0.5,fontFamily:"'Share Tech Mono',monospace"}}>NEW</span>}</>}
            </div>;})}
          </>}
          <div style={{height:12}}/>
        </div>
        {sidebarOpen&&<div style={{padding:"10px 8px 12px",borderTop:"1px solid #0a0a0a",background:"#020202",flexShrink:0}}>
          ...
          {([["Intel","Claude AI + Web","#ef4444"],["Crypto","CoinGecko API","#f97316"],["Stocks","Yahoo Finance","#22c55e"],["News","Google News RSS","#818cf8"],["Videos","YouTube RSS","#38bdf8"]] as [string,string,string][]).map(([n,s,c])=><div key={n} style={{display:"flex",alignItems:"center",gap:9,padding:"6px 9px",borderRadius:4,marginBottom:3,background:"#050505",border:"1px solid #0d0d0d"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=c+"33";e.currentTarget.style.background="#090909";}} onMouseLeave={e=>{e.currentTarget.style.borderColor="#0d0d0d";e.currentTarget.style.background="#050505";}}>
            <div style={{width:6,height:6,borderRadius:2,background:c,opacity:0.85,flexShrink:0}}/>
            <div style={{flex:1,minWidth:0}}><div style={{fontSize:9,fontWeight:600,color:"#6a6a6a",fontFamily:"'Share Tech Mono',monospace"}}>{n}</div><div style={{fontSize:7.5,color:"#2a2a2a",marginTop:1,fontFamily:"'Share Tech Mono',monospace"}}>{s}</div></div>
          </div>)}
          {lastUpdated&&<div style={{marginTop:6,paddingLeft:4,fontSize:7.5,color:"#1e1e1e",fontFamily:"'Share Tech Mono',monospace",display:"flex",alignItems:"center",gap:5}}><div style={{width:3,height:3,borderRadius:1,background:"#22c55e"}}/>{lastUpdated.toLocaleTimeString("en-US",{hour12:false})}</div>}
        </div>}
        {!sidebarOpen&&<div style={{padding:"10px 0 12px",borderTop:"1px solid #0a0a0a",display:"flex",flexDirection:"column",alignItems:"center",gap:5}}>{["#ef4444","#f97316","#22c55e","#818cf8","#38bdf8"].map((c,i)=><div key={i} style={{width:5,height:5,borderRadius:2,background:c,opacity:0.4}}/>)}</div>}
      </div>}

      {/* Main content */}
      <div style={{flex:1,overflow:"auto",padding:isMobile?"10px 10px 70px":"14px",scrollbarWidth:"thin",scrollbarColor:"#1c1c1c #000"}}>
        ...
        {page==="dashboard"&&intel&&<div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 270px",gap:14}}>
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(4,1fr)",gap:8}}>
              {[{label:"ACTIVE EVENTS",value:(intel.events||[]).length,color:"#ef4444"},{label:"CRITICAL",value:(intel.events||[]).filter((e:Event)=>e.severity==="critical").length,color:"#f97316"},{label:"DEFCON",value:intel.defcon||3,color:"#eab308"},{label:"TRENDING",value:trendingTabs.length,color:"#f97316"}].map((s,i)=><div key={i} style={{padding:"12px 14px",borderRadius:4,background:"#0a0a0a",border:"1px solid #1c1c1c",borderTop:`2px solid ${s.color}`}}><div style={{fontSize:7,color:"#333",letterSpacing:3,marginBottom:7,fontFamily:"'Share Tech Mono',monospace"}}>{s.label}</div><div style={{fontSize:isMobile?22:28,fontWeight:900,color:s.color,lineHeight:1,fontFamily:"'Share Tech Mono',monospace"}}>{s.value}</div></div>)}
            </div>
            <div style={{display:"grid",gridTemplateColumns:isMobile?"repeat(3,1fr)":"repeat(5,1fr)",gap:8}}>
              {crypto.slice(0,isMobile?3:5).map(c=>{const up=c.change>=0;return<div key={c.id} onClick={()=>{setPage("markets");setSelectedCoin(c.id);}} style={{padding:"9px 11px",borderRadius:4,background:"#0a0a0a",border:"1px solid #1c1c1c",cursor:"pointer"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=c.color;}} onMouseLeave={e=>{e.currentTarget.style.borderColor="#1c1c1c";}}>
                ...
                <div style={{fontSize:isMobile?10:12,fontWeight:800,color:"#d4e4f4",fontFamily:"'Share Tech Mono',monospace",marginBottom:2}}>${fmt(c.price,c.price>100?0:2)}</div>
            ...
            {intel.summary&&<div style={{padding:"11px 14px",borderRadius:4,background:"#0a0a0a",border:"1px solid #1c1c1c",borderLeft:"3px solid #f97316",display:"flex",gap:10}}>
              ...
              <div style={{flex:1}}><div style={{fontSize:7,letterSpacing:3,color:"#333",marginBottom:6,fontFamily:"'Share Tech Mono',monospace"}}>AI SITUATION ASSESSMENT</div><AITextBlock text={intel.summary} color="#8aaccc" fontSize={10}/></div>
            </div>}
            <div style={{background:"#050505",border:"1px solid #1c1c1c",borderRadius:4,overflow:"hidden",minHeight:isMobile?220:320}}>
            ...
            {trendingTabs.length>0&&<div style={{background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:4,overflow:"hidden"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(auto-fill,minmax(175px,1fr))",gap:0}}>
                {trendingTabs.map(t=><div key={t.id} onClick={()=>setPage(`trend:${t.id}`)} style={{padding:"11px 13px",borderRight:"1px solid #0f0f0f",borderBottom:"1px solid #0f0f0f",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background="#0f0f0f"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                  <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:6}}><span style={{fontSize:8,color:t.color,fontFamily:"'Share Tech Mono',monospace",fontWeight:900}}>{t.icon}</span><div style={{flex:1,minWidth:0}}><div style={{fontSize:9,color:t.color,fontWeight:700,fontFamily:"'Share Tech Mono',monospace",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>[{t.flag}] {t.label}</div><div style={{fontSize:7.5,color:"#333"}}>{t.eventCount} events</div></div>{t.isNew&&<span style={{fontSize:6.5,background:t.color,color:"#000",padding:"1px 4px",borderRadius:2,fontWeight:700,flexShrink:0}}>NEW</span>}</div>
        ...
        {/* Mobile dashboard — secondary panel items shown inline */}
        {page==="dashboard"&&intel&&isMobile&&<div style={{display:"flex",flexDirection:"column",gap:10,marginTop:10}}>
          ...
          <div style={{background:"#0a0a0a",border:"1px solid #1c1c1c",borderRadius:4,display:"flex",flexDirection:"column",overflow:"hidden",minHeight:200}}>
            <div style={{padding:"8px 12px",borderBottom:"1px solid #111",fontSize:7,letterSpacing:3,color:"#333",display:"flex",justifyContent:"space-between",alignItems:"center",fontFamily:"'Share Tech Mono',monospace"}}><span>LIVE EVENT FEED</span><div style={{display:"flex",gap:4,alignItems:"center"}}><div style={{width:4,height:4,background:"#ef4444",animation:"pulse 1s infinite"}}/><span style={{fontSize:7,color:"#ef4444",letterSpacing:2}}>LIVE</span></div></div>
    ...
    {trendingTabs.filter(t=>t.isNew).slice(0,1).map(t=><div key={t.id} style={{position:"fixed",bottom:isMobile?70:20,right:16,zIndex:9998,padding:"11px 14px",borderRadius:4,background:"#0a0a0a",border:`1px solid ${t.color}`,borderLeft:`3px solid ${t.color}`,animation:"fadeUp .3s ease",display:"flex",gap:10,alignItems:"center",maxWidth:280}}>
